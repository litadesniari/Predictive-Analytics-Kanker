...
| ------------------------- | ----------------- | ---------------- | ------------------ | ----------------- | --------------- | -------------- | ----------------- | ---------------- |
| Random Forest (RF)        | 0.828794          | 0.836576         | 0.852031           | 0.849174          | 0.828794        | 0.836576       | 0.834236          | 0.840283         |
| Decision Tree (DT)        | 0.739300          | 0.758755         | 0.794994           | 0.797607          | 0.739300        | 0.758755       | 0.750555          | 0.768024         |
...
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Random Forest (RF)**        | - Model ini sangat efektif untuk **memprediksi apakah seseorang menderita kanker hanya dengan data medis dasar** (usia, jenis kelamin, riwayat merokok, hasil lab).<br>- Dengan **Recall 83.66%**, model ini dapat mendeteksi sebagian besar kasus kanker, yang sangat penting dalam **sistem pendukung keputusan medis** (prioritas tinggi untuk deteksi kasus positif). | - **Recall tertinggi**: membantu mendeteksi sebagian besar pasien kanker.<br>- **Akurasi tinggi** dan **stabilitas prediksi** menjadikannya pilihan terbaik untuk **praktik klinis**.<br>- Cocok untuk penggunaan di **sistem screening kanker**. | - Tidak sebaik **Decision Tree** dalam hal **interpretabilitas**.<br>- Mungkin tidak seefisien dalam **kecepatan prediksi** jika dataset lebih besar.                                                                                                                             |
| **Decision Tree (DT)**        | - Model ini memberikan **keputusan yang mudah dipahami oleh tenaga medis**, yang penting untuk **transparansi** dalam diagnosis.<br>- Meskipun sedikit lebih rendah daripada Random Forest, model ini masih dapat mendeteksi sebagian besar kasus kanker dengan **Recall 75.88%**.                                                                                        | - Model yang **mudah dipahami** oleh profesional medis.<br>- Dapat menjadi **solusi baik untuk sistem dengan keterbatasan interpretabilitas**.<br>- **Peningkatan signifikan** setelah tuning.                                                    | - Rentan terhadap **overfitting**, terutama pada data dengan **variasi besar**.<br>- **Recall lebih rendah** daripada RF, sehingga ada potensi **melewatkan beberapa kasus kanker**.                                                                                              |
| **Logistic Regression (LR)**  | - Model ini memberikan **presisi tinggi** tetapi dengan **Recall lebih rendah**. Ini berarti model ini **lebih berhati-hati dalam mendeteksi kanker**, yang mengarah pada **melewatkan beberapa pasien kanker** (false negatives).<br>- Cocok digunakan sebagai **baseline model** dalam **kondisi terbatas**.                                                            | - **Precision tinggi**: sedikit kesalahan dalam prediksi positif.<br>- **Sederhana dan cepat** dalam implementasi.<br>- Cocok digunakan untuk **pengujian awal atau model baseline**.                                                             | - **Recall rendah**: berisiko melewatkan pasien kanker (false negatives), yang sangat berbahaya dalam konteks medis.<br>- Tidak cukup kuat untuk menangani **kompleksitas data kanker** jika dibandingkan dengan RF atau DT.                                                      |
| **K-Nearest Neighbors (KNN)** | - **KNN memiliki hasil yang lebih rendah** dan kurang efektif dalam **mendeteksi kanker** dibandingkan model lain, yang mengarah pada **akurasi dan recall yang lebih rendah**.<br>- Model ini tidak ideal untuk sistem pendukung keputusan medis yang memerlukan **akurasi dan sensitivitas tinggi** dalam mendeteksi kanker.                                            | - Memiliki **Precision dan Recall yang seimbang**.<br>- **Sederhana untuk dipahami dan diimplementasikan**.<br>- Bisa berguna jika data sangat sederhana dan kecil.                                                                               | - **Akurasi rendah** dan **Recall rendah** mengindikasikan bahwa model ini **kurang efektif untuk mendeteksi kanker**.<br>- **Kurang efisien dalam menangani data dengan banyak fitur atau noise**.<br>- Tidak dapat diandalkan dalam **skala besar** atau aplikasi medis kritis. |
